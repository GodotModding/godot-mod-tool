...
onready var min_size_y := text_edit.rect_min_size.y as int
...
func _on_dragged(offset: int) -> void:
	...
	if new_size < min_size_y:
		text_edit.rect_min_size.y = min_size_y
	else:
		text_edit.rect_min_size.y = new_size
	...
	if event is InputEventMouseButton:
		if event.button_index == BUTTON_LEFT:
			if event.pressed and not is_pressed:
				is_pressed = true
				previous_size = text_edit.rect_min_size.y
			else:
				is_pressed = false
