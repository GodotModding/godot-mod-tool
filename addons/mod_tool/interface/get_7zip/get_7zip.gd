...
func install(download_file_path: String) -> void:
	if mod_tool_store.current_os == "windows":
		# Run the setup
		var _exit_code := OS.execute("cmd.exe", ["/c", "%s /S /D=\"%s\"" % [download_file_path, mod_tool_store.path_global_seven_zip_base_dir]])
	else:
		# Unpack install files
		var _exit_code := OS.execute('bash',["-c", "tar -xf %s -C %s" % [download_file_path, mod_tool_store.path_global_seven_zip_base_dir]])
...
func _on_HTTPRequest_request_completed(result: int, response_code: int, headers: PackedStringArray, body: PackedByteArray) -> void:
	...
	var file := FileAccess.open(path_global_seven_zip_download_file, FileAccess.WRITE)
	if not file:
		ModToolUtils.output_error("Failed to write downloaded file. Error code: \"%s\"." % FileAccess.get_open_error())
