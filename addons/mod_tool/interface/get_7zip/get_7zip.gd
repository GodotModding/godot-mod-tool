...
func _on_HTTPRequest_request_completed(result: int, response_code: int, headers: PoolStringArray, body: PoolByteArray) -> void:
	...
	var path_global_seven_zip_download_file := ModToolStore.path_global_seven_zip_base_dir + current_download_suffix

	if not _ModLoaderFile.dir_exists(ModToolStore.path_global_seven_zip_base_dir):
		ModToolUtils.make_dir_recursive(ModToolStore.path_global_seven_zip_base_dir)
