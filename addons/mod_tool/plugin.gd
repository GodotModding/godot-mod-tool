...
func _enter_tree() -> void:	
	...
	editor_interface.get_editor_main_screen().call_deferred("add_child", tools_panel, true)
	_make_visible(false)
...
func _get_plugin_icon():
	return editor_interface.get_base_control().get_theme_icon(&"Tools", &"EditorIcons")


func connect_to_script_editor() -> void:
	editor_interface.get_script_editor().editor_script_changed.connect(ModToolUtils.reload_script.bind(mod_tool_store))
